...
Public Class PURCHASE_INVOICE
    ...
    Private Sub PURCHASE_INVOICE_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        sqlstr = " Exec [spMenu_Access] '" & siduser & "','PURCHASE_INVOICE' "
        ...
        Me.txtPIDate.Text = FormatTanggal_View(Date.Now)
        Me.KeyPreview = True
    ...
    Public Sub Search_Data(ByVal PIID As String)
        tunggu_arrow()
        ...
        Dim SqlHeader As String = "[spPI_Header] '" & Trim(PIID) & "'"
        ...
        If dtHeader.Rows.Count > 0 Then
            ...
            GridControl(False)
            Me.txtPIID.Text = Trim(PIID)
            Me.txtPINo.Text = dtHeader.Rows(0).Item("PINo")
            Me.txtPIDate.Text = dtHeader.Rows(0).Item("PIDate")
    ...
    Private Sub Bind_Detail(ByVal PIID As String)
        If cn5.State = ConnectionState.Open Then cn5.Close()
        cn5 = ax.cntsvr
        cmd5.Connection = cn5
        sqlstr = "[spPI_Detail] '" & PIID & "'"
        ...
        Do Until dtRead5.Read = False
            ...
            Me.dgDetail.Item("POID", row).Value = dtRead5!PRNo
            Me.dgDetail.Item("QTYPO", row).Value = dtRead5!QtyPR
    ...
    Private Sub SetDefaultValue()
        ...
        Me.txtPINo.BackColor = Color.FromArgb(235, 235, 224)
        ...
        If Me.dgDetail.RowCount > 0 Then
            If Me.dgDetail.Rows(Me.dgDetail.CurrentRow.Index).Selected = True Then
                ...
                If Me.dgDetail.Focused = True Then
                    ...
                    If sresponse = CStr(MsgBoxResult.Yes) Then
                        Try
                            Dim SqlDelItem As String = "spPR_DeleteItem '" & Me.txtPINo.Text & "','" & RowNo & "'"
    ...
    Private Sub Save_Header()
        ...
        cmdSave.Parameters.Add("@PIID", SqlDbType.VarChar).Value = Me.txtPIID.Text
        cmdSave.Parameters.Add("@PoNo", SqlDbType.VarChar).Value = Me.txtPINo.Text
        cmdSave.Parameters.Add("@PODate", SqlDbType.Date).Value = FormatTanggal_Save(Me.txtPIDate.Text)
    ...
    Private Sub Save_Detail()
        For Each row As DataGridViewRow In dgDetail.Rows
            ...
            If row.Cells("ITEM").Value <> "" Then
                ...
                cmdSaveDetail.Parameters.Add("@PIID", SqlDbType.VarChar).Value = Me.txtPIID.Text
    ...
    Private Sub isi_data(ByVal PIID As String, ByVal PONo As String, ByVal PODate As String)

        Dim ds As New DataSet
        pesan = "GRID LIST"
        Try
            Dim SqlStrEmp As String = "[spPO_List] '" & PIID & "', '" & PONo & "','" & PODate & "'"
    ...
    Private Sub dgfind_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgfind.CurrentCellChanged, dgfind.CellClick
        ...
        Else
            ...
            Else
                pID = IIf(IsDBNull(dgfind.Item("PIID", dgfind.CurrentRow.Index).Value), "", dgfind.Item("PIID", dgfind.CurrentRow.Index).Value)
                sketproses = "FIND"
                select_cell = "CELLSELECT"
                Search_Data(pID)
            End If
    ...
    Private Sub Bind_Detail_PR(ByVal PRCollection As String)
        ...
        cn5 = ax.cntsvr
        cmd5.Connection = cn5
        sqlstr = "[spPO_Detail_ItemPR] '" & Me.txtPIID.Text & "','" & PRCollection & "'"
        cmd5.CommandText = sqlstr
        dtRead5 = cmd5.ExecuteReader
        row = 0
    ...
    Private Sub PRINT_PO_Click(sender As Object, e As EventArgs) Handles PRINT_PO.Click
        Try
            ...
            crParameterDiscreteValue.Value = Me.txtPIID.Text.Trim
